...
def mkdirs(pre_path,parm_name=None):
    try:
...
def eqratio_v_parm_bar(df,plot_parm,pre_path,parm_name=None,post_path='',save=True):
    # allcell_eq_ratio(df)
    fig,ax1=plt.subplots()
    cell_ratios=['Tpos_eq','Tpro_eq','Tneg_eq']
    cell_colors=['tab:green','tab:blue','tab:red']
    cell_labels=['T+','Tp','T-']
    df.plot.bar(x=plot_parm,y=cell_ratios,color=cell_colors,stacked=True,ax=ax1)
    # ax1.set_ylim(0,1.1)
    ax1.legend()
    ax1.set_ylabel('Final abundance')
    ax1.set_xlabel(plot_parm)
    # plt.xticks(rotation=45)
    ax1.legend(cell_labels)
    if save:
        fig.savefig('/figures/'+pre_path+parm_name+'-'+post_path+'-bar_fin_num.svg')
    fig.clf()
    plt.close(fig)
    
def eq_values(pre_path,parm_name,parm_array,parm_format='{:.2E}',post_path=None,save=True,parm_name_array=None,ttp=False,limit=None):
    lis=[]
    ...
    if isinstance(parm_array[0],(list,pd.core.series.Series,np.ndarray)): #If the parameter explored is multidimensional
        for parm in parm_array:
            ...
            df=pd.read_csv('/data/'+pre_path+parm_name+'/'+string+'.csv')
            ttp_arr=[]
            if ttp:
        ...
        df=pd.DataFrame(lis,columns=columns)
    if save:
        df.to_csv('/analysed_data/'+pre_path+parm_name+'-eq_values.csv',index=False)
    return df
    
"""
######################################################
####Other functions used elsewhere in the analysis####
######################################################

**These have not been used directly for the plots in the main text, but were used with the appropriate data for returning the steady state values for every parameter combination that were then collated into a single CSV file shared separately. Functions given here were also used to generate the supplementary time series figures by passing the appropriate data files and adjusting the export path depending on where figures needed to be saved.
In most cases, function calls include paths to directories whose structure reflects our internal organisation. These will need to be edited according to the directory structure of the system that this is run in.
...
def timeseries(pre_path,parm_name,parm_array,parm_format='{:.2E}',post_path='',plot_Tpos=True,plot_Tpro=True,plot_Tneg=True,plot_o2=True,plot_test=True,plot_tot=False,custom_title=None,save=True):
    ...
    fig.tight_layout()
    if save:
        fig.savefig('../../figures/'+pre_path+parm_name+'/timeseries.svg')
    fig.clf()
    plt.close(fig)
...
def eqratio_v_parm(df,plot_parm,pre_path,parm_name,post_path='',plot_Tpos=True,plot_Tpro=True,plot_Tneg=True,save=True,style_parm=None):
    ...
    fig,ax1=plt.subplots()
    if plot_Tpos:
        sns.lineplot(data=df,x=plot_parm,y='Tpos_eq',color='tab:green',style=style_parm,label="T+",ax=ax1)
    if plot_Tpro:
        sns.lineplot(data=df,x=plot_parm,y='Tpro_eq',color='tab:blue',style=style_parm,label="Tp",ax=ax1)
    if plot_Tneg:
        sns.lineplot(data=df,x=plot_parm,y='Tneg_eq',color='tab:red',style=style_parm,label="T-",ax=ax1)
    ax1.set_ylim(0,10000)
    ax1.legend()
    ax1.set_ylabel('Final abundance')
    # ax1.set_xlabel(plot_parm)
    if save:
        fig.savefig('../../figures/'+pre_path+parm_name+'-'+post_path+'-eqabundance-vs-res-eff'+'.svg')
    fig.clf()
    plt.close(fig)
"""